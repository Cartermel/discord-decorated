...
describe("ParseNumberTransformer", () => {
  const transformer = new ParseNumberTransformer();
  ...
  it("Should parse strings to numbers", async () => {
    await expect(
      transformer.transform(["1", "2", "3.14", "12339125.123312"])
    ).resolves.toStrictEqual([1, 2, 3.14, 12339125.123312]);
  });
